...
echo Last updated September 27, 2025 \(rev. B\)
echo ----------------------------------------------------------------------------------------------
...
# Hide password input
read -rsp "$username password: " userpassword
echo
...
# Personal default: America/Denver (press Enter to accept)
echo
...
# If user typed a full path or a short TZ, normalize into a valid /usr/share/zoneinfo path
# We keep your interactive explorer as a fallback if needed.
if [ -f "$timezone" ]; then
    # user provided full file path
    timezone="$timezone"
elif [ -f "$zroot/$timezone" ]; then
    timezone="$zroot/$timezone"
else
    # Fall back to interactive picker if invalid / incomplete
    while true; do
        show_tz_list
        echo
        read -rp "Timezone (or Enter for $DEFAULT_TZ): " tz_try
        if [ -z "$tz_try" ]; then
            tz_try="$DEFAULT_TZ"
        fi
        ...
        while [ -d "$zroot/$timezone" ]; do
            ...
            echo
            read -rp "$timezone subset (or ?): " zone
            case "$zone" in
