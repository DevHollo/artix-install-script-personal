...
# FIX: timezone link should be forced, and set hardware clock
...
# shell configuration (PERSONALIZED: fish login shell)
pacman -Sy fish zsh zsh-autosuggestions zsh-syntax-highlighting --noconfirm
...
# Hollo default: fish login shell
chsh -s /usr/bin/fish "$username"

# Keep dash as /bin/sh for system scripts (unchanged)
ln -sfT dash /usr/bin/sh

# fish config (PERSONAL)
mkdir -p /home/"$username"/.config/fish
cat << 'EOF' > /home/"$username"/.config/fish/config.fish
# Hollo default fish config
...
chmod 700 /home/"$username"/.config/fish
chmod 600 /home/"$username"/.config/fish/config.fish
...
if [ "$formfactor" == 1 ] || [ "$formfactor" == 2 ] || [ "$formfactor" == 3 ]; then
    ...
    install -m 0600 -o "$username" -g users ./config-files/plasma/gruvboxDarkKonsole.colorscheme /home/"$username"/.local/share/konsole/gruvboxDarkKonsole.colorscheme
